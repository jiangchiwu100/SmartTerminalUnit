...
static uint16_t error_cn = 0;
/* static function define ---------------------------------------*/
static void set_filename(uint8_t *file, uint32_t file_size)
{
	rt_memcpy(g_filename, file, file_size);
}
...
int open_tftp_socket(uint8_t sock)
{
	...
	if(sd != 1) 
	{
		rt_kprintf("[%s] socket error\r\n", __func__);
		return -1;
	}
	do
	{
		sck_state=getSn_SR(sd);
//		rt_kprintf(" %2x",getSn_SR(sd));
	} 	
...
void send_tftp_rrq(uint8_t *filename, uint8_t *mode, TFTP_OPTION *opt, uint8_t opt_len)
{
	...
	*((uint16_t *)pkt) = htons(TFTP_RRQ);
	...
	pkt += rt_strlen((char *)filename) + 1;
	strcpy((char *)pkt, (const char *)mode);
	pkt += rt_strlen((char *)mode) + 1;

	for(i = 0 ; i < opt_len ; i++) {
		strcpy((char *)pkt, (const char *)opt[i].code);
		pkt += rt_strlen((char *)opt[i].code) + 1;
		strcpy((char *)pkt, (const char *)opt[i].value);
		pkt += rt_strlen((char *)opt[i].value) + 1;
	}
	...
	set_filename(filename, rt_strlen((char *)filename) + 1);
	tftp_reg_timeout();
#ifdef __TFTP_DEBUG__
	rt_kprintf(">> TFTP RRQ : FileName(%s), Mode(%s)\r\n", filename, mode);
#endif
}
...
void send_tftp_ack(uint16_t block_number)
{
	...
	*((uint16_t *)pkt) = htons(block_number);
	...
	tftp_reg_timeout();
#ifdef __TFTP_DEBUG__
//	rt_kprintf(">> TFTP ACK : Block Number(%d)\r\n", block_number);
#endif
}
...
void send_tftp_error(uint16_t error_number, uint8_t *error_message)
{
	...
	*((uint16_t *)pkt) = htons(error_number);
	...
	pkt += rt_strlen((char *)error_message) + 1;
	...
	tftp_reg_timeout();
#ifdef __TFTP_DEBUG__
	rt_kprintf(">> TFTP ERROR : Error Number(%d)\r\n", error_number);
#endif
}
#endif
...
void recv_tftp_data(uint8_t *msg, uint32_t msg_len)
{
	...
	#ifdef __TFTP_DEBUG__
//	rt_kprintf("<< TFTP_DATA : opcode(%d), block_num(%d)\r\n", data->opcode, data->block_num);
	#endif
...
void recv_tftp_packet(uint8_t *packet, uint32_t packet_len, uint32_t from_ip, uint16_t from_port)
{
	...
	{
		#ifdef __TFTP_DEBUG__
		rt_kprintf("[%s] Server IP faults\r\n", __func__);
		rt_kprintf("from IP : %08x, Server IP : %08x\r\n", from_ip, get_server_ip());
		#endif
	...
	if((get_tftp_state() == STATE_WRQ) || (get_tftp_state() == STATE_RRQ)) 
	{
		...
		#ifdef __TFTP_DEBUG__
		rt_kprintf("[%s] Set Server Port : %d\r\n", __func__, from_port);
		#endif
	}
	switch(opcode)
	{
...
int TFTP_run(void)
{
	...
	{
	#ifdef __TFTP_DEBUG__
		rt_kprintf("[%s] recv_udp_packet error\r\n", __func__);
	#endif
...
void TFTP_read_request(uint32_t server_ip, uint8_t *filename)
{
	...
	#ifdef __TFTP_DEBUG__
	rt_kprintf("[%s] Set Tftp Server : %x\r\n", __func__, server_ip);
	#endif
	g_progress_state = TFTP_PROGRESS;
