...
void dfs_test(void)
{
	...
	rt_uint8_t buf1[] = {1,2,3,4,5,6};
	rt_uint8_t buf2[6];

    rt_kprintf("dfs test start!\r\n");

//	fd = mkdir("/sojo/soe", 0x777);
//	if (fd < 0)
//	{
...
//	}
	...
	fd = open("/sojo/fevent.txt", O_RDWR | O_CREAT, 0);

	if (fd < 0)
	{
		rt_kprintf("open Myfile failed!\r\n"); 
	}
	else
	{
		...
		fd = open("/sojo/fevent.txt", O_RDONLY, 0);
		
		if (read(fd, buf2, sizeof(buf2)))
		{
			rt_kprintf("readbuf = {");
		...
		{
			rt_kprintf("read /sojo/fevent.txt failed \r\n"); 
		}
		
		close(fd);
	}

    stat("/sojo/fevent.txt", &buf);	 
	rt_kprintf("st_size = %d \r\n", buf.st_size); 
	rt_kprintf("st_mtime = %d \r\n", buf.st_mtime);
	rt_kprintf("st_blksize = %d \r\n", buf.st_blksize); 
	rt_kprintf("st_dev = %d \r\n", buf.st_dev);	
	rt_kprintf("st_mode = %d \r\n", buf.st_mode);
    p = localtime(&buf.st_mtime);	
	
	rt_kprintf("%d-%02d-%02d ", (1900 + p->tm_year), (p->tm_mon), (p->tm_mday));
	rt_kprintf("%02d:%02d:%02d \n", (p->tm_hour), (p->tm_min), (p->tm_sec));	
	
	rt_kprintf("dfs test finish!\r\n");
}
RTM_EXPORT(dfs_test);
FINSH_FUNCTION_EXPORT(dfs_test, test the file system);
FINSH_FUNCTION_EXPORT_ALIAS(dfs_test, dfs, test the file system);
/* @} */
