...
char NetGetchar(void)
{
    ...
    do
    {
        char ch = FifoCharDequeue(FinshReceiveFifoHandle);
        if (ch)
        {
            return ch;
        }
...
/**
  ...
  * @param : data 发送的数据
  * @param : lenth 发送的数据长度
  * @param : remoteIP 远程IP地址的数组
  * @param : remotePort 远程端口号
  * @return: -1：发送错误	其他：发送完成的字节数
  * @update: [2018-10-09][李  磊][创建]
  */

int32_t UDP_SocketSendString(int32_t socket, uint8_t* data, uint32_t lenth, uint8_t* remoteIP, uint32_t remotePort)
{
	int32_t sendNum = 0;
	struct hostent* host;
	...
	//remoteAddress.sin_addr.s_addr = COMBINE_UINT32(remoteIP[3], remoteIP[2], remoteIP[1], remoteIP[0]);
...
uint32_t UdpSocketInit(uint32_t* socketNum, struct sockaddr* socketAddress)
{
	...
	lwip_setsockopt(g_UDP_ServeSocket,SOL_SOCKET,SO_BROADCAST,( void *)&optval,sizeof(optval));
	/* 绑定socket到服务端地址 */
