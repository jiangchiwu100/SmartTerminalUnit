...
//        netmask.addr = inet_addr(RT_LWIP_MSKADDR);
        ...
        for(i=0;i<4;i++)
        {
            ipaddr.addr = ipaddr.addr<<8;
            ipaddr.addr |= (g_EthDP83848.ip[3-i]&0xff);
            gw.addr = gw.addr<<8;
            gw.addr |= (g_EthDP83848.gateway[3-i]&0xff);
            netmask.addr = netmask.addr<<8;
            netmask.addr |= (g_EthDP83848.netmask[3-i]&0xff);
        }

        netifapi_netif_set_addr(netif_default, &ipaddr, &netmask, &gw);
    }
#endif
    LWIP_PRINTF("lwIP-%d.%d.%d initialized!\n", LWIP_VERSION_MAJOR, LWIP_VERSION_MINOR, LWIP_VERSION_REVISION);

    return(0);
}
INIT_ENV_EXPORT(lwip_system_init);
