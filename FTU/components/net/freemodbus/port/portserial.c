...
static void prvvUARTRxISR(UCHAR ucLen);
        ...
        eMBParity eParity)
{
    ...
    /* set serial name */
    if (ucPORT == 1) {
#if defined(RT_USING_USART1) || defined(RT_USING_REMAP_UART1)
        ...
        extern struct rt_serial_device serial7;
        serial = &serial7;
#endif
    }
    ...
    /* software initialize */
    rt_event_init(&event_serial, "modbus serial", RT_IPC_FLAG_PRIO);
...
BOOL xMBPortSerialGetByte(CHAR * pucByte, UCHAR ucLen)
{
    serial->parent.read(&(serial->parent), 0, pucByte, ucLen);
    return TRUE;
}
 ...
 * character.
 */
void prvvUARTRxISR(UCHAR ucLen)
{
    pxMBFrameCBByteReceived(ucLen);
}
