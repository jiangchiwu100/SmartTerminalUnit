/**
  ...
  * @file:      distribution_interface_goose.c
  * @brief:     分布式接口函数,基于goose的接口
  * @version:   V0.0.0 
  * @author:    Zhang Yufei
  * @date:      2018-08-23
  * @update:   
...
#include "distribution_interface_goose.h"   
...
static inline bool IsFault(FaultDealHandle* handle)
{
    return  (DeviceIndicate_getBooleanStatus(GetDeviceIndicate(handle), DEVICE_IED_FAULT));
}
/**
* @brief :是否为分位
* @param  RemovalHandle* handle 控制句柄
...
{
	//TODO 这里是单点
    return  !DeviceIndicate_getBooleanStatus(GetDeviceIndicate(handle), DEVICE_IED_SWITCH_POS);
}
/**
* @brief :是否为合位
* @param  RemovalHandle* handle 控制句柄
...
static inline bool IsTrigger(FaultDealHandle* handle)
{
    ...
    for (uint8_t i = 0; i < distributionArea->areaCount; i++)
    {
    	...
    	{
    		result = DeviceIndicate_getBooleanStatus(
    				GetDeviceIndicateBySwitch(area->areaSwitch[k]), DEVICE_IED_FAULT);
    		if (result)
    		{
    			return true;
    		}
