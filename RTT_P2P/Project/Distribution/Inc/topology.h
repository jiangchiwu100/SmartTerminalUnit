...
//增添
extern ErrorCode  AddTopologyMember(const uint8_t data[], uint8_t len,  List* topologyList);
extern ErrorCode  AddMemberByTopology(TopologyMessage*  topologyMessage, List* topologyList);
extern void DeleteTopologyListNodeById(List* topologyList, uint32_t id);


//查找相关
extern ErrorCode FindSwitchNodeByID(const List* listNeighboorSwitch, uint32_t id, SwitchProperty** find);
extern ErrorCode FindTopologyNodeByID(List* listNeighboorSwitch, uint32_t id, TopologyMessage** find);
...
//转换相关
extern ErrorCode GetNeighboorSwitch(List* neighbourSwitchList, TopologyMessage* local, const  List* listNeighboor, List* listNeighboorSwitch);
extern ErrorCode GetSwitchList(const  List* listTopology, List* listSwitch);


//路经有关
extern ErrorCode BreadthFirstPath(const List* switchList, const SwitchProperty* start, BFSHelper** helper);
extern ErrorCode PathTo(const List* switchList, const BFSHelper* helper, const SwitchProperty* start, const SwitchProperty* end);
extern ErrorCode FindPath(const List* switchList, const SwitchProperty* start, const SwitchProperty* end, BFSHelper** helper);
...
//配电区域相关
extern ErrorCode GetPowerDistributionArea(TopologyMessage* local,List* listNeighboorSwitch, List* listArea);
extern ErrorCode ExtractPowerAreaFromList(SwitchProperty* switchRef, DistributionStation* distriStation, uint8_t len);
extern void FreeAreaList(List* areaList, uint8_t len);

extern ErrorCode CheckNeighboorTopologyCompleted(SwitchProperty* curent, List* list, bool* result);
...
extern ErrorCode ConnectPath_ResetUpdateFlag(List* list);
extern ErrorCode ConnectPath_CheckIsUpdateComplete(List* list, bool* result);
extern ErrorCode ConnectPath_ResetUpdateFlag(List* list);
extern ErrorCode ConnectPath_MeetCondition(List* list, ConnectPath** pcp);
#ifdef	__cplusplus
}
#endif
