...
//        0xB829u);                               
    ...
    IOMUXC_SetPinMux(IOMUXC_GPIO_B1_10_ENET_REF_CLK,1);    //必须为1，否则网络工作不正常！                                                                   
    IOMUXC_SetPinMux(IOMUXC_GPIO_B1_14_ENET_MDC,0);                                   
    IOMUXC_SetPinMux(IOMUXC_GPIO_B1_15_ENET_MDIO,0);

    
    //设置复位和中断引脚
    //高转换速度,驱动能力为R0/5,速度为100Mhz，开路输出，使能pull/keepr
	//选择pull功能，上拉100K Ohm，关闭Hyst                             
	...
	IOMUXC_GPR->GPR1|=1<<23;		//ipg clk 总开启
}
